export const database = [
    {
        title: "Money Heist 1",
        label: "NETFLIX",
        year: "1980",
        type: "TV",
        description: `A criminal mastermind who goes by "The Professor" has a plan
        to pull off the biggest heist in recorded history — to print billions of euros in the Royal Mint of
        Spain. To help him carry out the ambitious plan, he recruits eight people with certain abilities
        and who...1`,
        tomato: "93%",
        audience: "81%",
        genres: "Crime, Thriller, Heist",
        cast: "Úrsula Corberó, Álvaro Morte, Alba Flores, Itziar Ituño",
        img: "dist/img/rounded_rectangle_2_17.png",
        preview: "dist/img/galery/rectangle_5_copy_298.png"
    },
    {
        title: "Some shit 2",
        label: "AMAZON Prime",
        year: "2012",
        type: "TV",
        description: `Бежал, бежал, упал. ожвыожпо ожооып вапцп щшогрлтлт лтлтолттл лтлдтлт рпмрпсмо пасасс`,
        tomato: "37%",
        audience: "31%",
        genres: "Crime, Heist",
        cast: "Úrsula Morte, Alba Flores, Itziar Habenskiy",
        img: "dist/img/rounded_rectangle_2_17.png",
        preview: "dist/img/galery/rectangle_5_copy_493.png"
    },
    {
        title: "Taykatuvan 3",
        label: "NETFLIX",
        year: "2020",
        type: "Movie",
        description: `Ракамакафон. Джа джа бинг бежал, бежал, упал. ожвыожпо ожооып вапцп щшогрлтлт лтлтолттл лтлдтлт рпмрпсмо пасасс`,
        tomato: "78%",
        audience: "21%",
        genres: "Crime, Heist",
        cast: "Alba Flores, Itziar Habenskiy",
        img: "dist/img/rounded_rectangle_2_17.png",
        preview: "dist/img/galery/rectangle_5_copy_110.png"
    },
    {
        title: "Money Heist 4",
        label: "NETFLIX",
        year: "2017",
        type: "TV",
        description: `A criminal mastermind who goes by "The Professor" has a plan
        to pull off the biggest heist in recorded history — to print billions of euros in the Royal Mint of
        Spain. To help him carry out the ambitious plan, he recruits eight people with certain abilities
        and who...1`,
        tomato: "93%",
        audience: "81%",
        genres: "Crime, Thriller, Heist",
        cast: "Úrsula Corberó, Álvaro Morte, Alba Flores, Itziar Ituño",
        img: "dist/img/rounded_rectangle_2_17.png",
        preview: "dist/img/galery/rectangle_5_copy_298.png"
    },
    {
        title: "Some shit 5",
        label: "AMAZON Prime",
        year: "2012",
        type: "TV",
        description: `Бежал, бежал, упал. ожвыожпо ожооып вапцп щшогрлтлт лтлтолттл лтлдтлт рпмрпсмо пасасс`,
        tomato: "37%",
        audience: "81%",
        genres: "Crime, Heist",
        cast: "Úrsula Morte, Alba Flores, Itziar Habenskiy",
        img: "dist/img/rounded_rectangle_2_17.png",
        preview: "dist/img/galery/rectangle_5_copy_493.png"
    },
    {
        title: "Taykatuvan 6",
        label: "NETFLIX",
        year: "2020",
        type: "Movie",
        description: `Ракамакафон. Джа джа бинг бежал, бежал, упал. ожвыожпо ожооып вапцп щшогрлтлт лтлтолттл лтлдтлт рпмрпсмо пасасс`,
        tomato: "78%",
        audience: "81%",
        genres: "Crime, Heist",
        cast: "Alba Flores, Itziar Habenskiy",
        img: "dist/img/rounded_rectangle_2_17.png",
        preview: "dist/img/galery/rectangle_5_copy_110.png"
    },
    {
        title: "Money Heist 7",
        label: "NETFLIX",
        year: "2017",
        type: "TV",
        description: `A criminal mastermind who goes by "The Professor" has a plan
        to pull off the biggest heist in recorded history — to print billions of euros in the Royal Mint of
        Spain. To help him carry out the ambitious plan, he recruits eight people with certain abilities
        and who...1`,
        tomato: "93%",
        audience: "81%",
        genres: "Crime, Thriller, Heist",
        cast: "Úrsula Corberó, Álvaro Morte, Alba Flores, Itziar Ituño",
        img: "dist/img/rounded_rectangle_2_17.png",
        preview: "dist/img/galery/rectangle_5_copy_298.png"
    },
    {
        title: "Some shit 8",
        label: "AMAZON Prime",
        year: "2012",
        type: "TV",
        description: `Бежал, бежал, упал. ожвыожпо ожооып вапцп щшогрлтлт лтлтолттл лтлдтлт рпмрпсмо пасасс`,
        tomato: "37%",
        audience: "81%",
        genres: "Crime, Heist",
        cast: "Úrsula Morte, Alba Flores, Itziar Habenskiy",
        img: "dist/img/rounded_rectangle_2_17.png",
        preview: "dist/img/galery/rectangle_5_copy_493.png"
    },
    {
        title: "Taykatuvan 9",
        label: "NETFLIX",
        year: "2020",
        type: "Movie",
        description: `Ракамакафон. Джа джа бинг бежал, бежал, упал. ожвыожпо ожооып вапцп щшогрлтлт лтлтолттл лтлдтлт рпмрпсмо пасасс`,
        tomato: "78%",
        audience: "81%",
        genres: "Crime, Heist",
        cast: "Alba Flores, Itziar Habenskiy",
        img: "dist/img/rounded_rectangle_2_17.png",
        preview: "dist/img/galery/rectangle_5_copy_110.png"
    },
    {
        title: "Money Heist 10",
        label: "NETFLIX",
        year: "2017",
        type: "TV",
        description: `A criminal mastermind who goes by "The Professor" has a plan
        to pull off the biggest heist in recorded history — to print billions of euros in the Royal Mint of
        Spain. To help him carry out the ambitious plan, he recruits eight people with certain abilities
        and who...1`,
        tomato: "93%",
        audience: "81%",
        genres: "Crime, Thriller, Heist",
        cast: "Úrsula Corberó, Álvaro Morte, Alba Flores, Itziar Ituño",
        img: "dist/img/rounded_rectangle_2_17.png",
        preview: "dist/img/galery/rectangle_5_copy_298.png"
    },
    {
        title: "Some shit 11",
        label: "AMAZON Prime",
        year: "2012",
        type: "TV",
        description: `Бежал, бежал, упал. ожвыожпо ожооып вапцп щшогрлтлт лтлтолттл лтлдтлт рпмрпсмо пасасс`,
        tomato: "37%",
        audience: "81%",
        genres: "Crime, Heist",
        cast: "Úrsula Morte, Alba Flores, Itziar Habenskiy",
        img: "dist/img/rounded_rectangle_2_17.png",
        preview: "dist/img/galery/rectangle_5_copy_493.png"
    },
    {
        title: "Taykatuvan 12",
        label: "NETFLIX",
        year: "2020",
        type: "Movie",
        description: `Ракамакафон. Джа джа бинг бежал, бежал, упал. ожвыожпо ожооып вапцп щшогрлтлт лтлтолттл лтлдтлт рпмрпсмо пасасс`,
        tomato: "78%",
        audience: "81%",
        genres: "Crime, Heist",
        cast: "Alba Flores, Itziar Habenskiy",
        img: "dist/img/rounded_rectangle_2_17.png",
        preview: "dist/img/galery/rectangle_5_copy_110.png"
    },
    {
        title: "Money Heist 13",
        label: "NETFLIX",
        year: "2017",
        type: "Movie",
        description: `A criminal mastermind who goes by "The Professor" has a plan
        to pull off the biggest heist in recorded history — to print billions of euros in the Royal Mint of
        Spain. To help him carry out the ambitious plan, he recruits eight people with certain abilities
        and who...1`,
        tomato: "93%",
        audience: "72%",
        genres: "Crime, Thriller, Heist",
        cast: "Úrsula Corberó, Álvaro Morte, Alba Flores, Itziar Ituño",
        img: "dist/img/rounded_rectangle_2_17.png",
        preview: "dist/img/galery/rectangle_5_copy_529.png"
    },
    {
        title: "Some shit 14",
        label: "AMAZON Prime",
        year: "2012",
        type: "TV",
        description: `Бежал, бежал, упал. ожвыожпо ожооып вапцп щшогрлтлт лтлтолттл лтлдтлт рпмрпсмо пасасс`,
        tomato: "37%",
        audience: "81%",
        genres: "Crime, Heist",
        cast: "Úrsula Morte, Alba Flores, Itziar Habenskiy",
        img: "dist/img/rounded_rectangle_2_17.png",
        preview: "dist/img/galery/rectangle_5_copy_413.png"
    },
    {
        title: "Taykatuvan 15",
        label: "NETFLIX",
        year: "1950",
        type: "Movie",
        description: `Ракамакафон. Джа джа бинг бежал, бежал, упал. ожвыожпо ожооып вапцп щшогрлтлт лтлтолттл лтлдтлт рпмрпсмо пасасс`,
        tomato: "78%",
        audience: "81%",
        genres: "Crime, Heist",
        cast: "Alba Flores, Itziar Habenskiy",
        img: "dist/img/rounded_rectangle_2_17.png",
        preview: "dist/img/galery/rectangle_5_copy_237.png"
    },
]